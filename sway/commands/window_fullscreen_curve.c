...
static bool set_curve(double c1x, double c1y, double c2x, double c2y) {
	...
	config->window_fullscreen_curve_c1x = c1x;
	config->window_fullscreen_curve_c1y = c1y;
	config->window_fullscreen_curve_c2x = c2x;
	config->window_fullscreen_curve_c2y = c2y;
	return true;
}

struct cmd_results *cmd_window_fullscreen_curve(int argc, char **argv) {
	struct cmd_results *error = NULL;
	if ((error = checkarg(argc, "window_fullscreen_curve", EXPECTED_AT_LEAST, 1))) {
		return error;
	}
	...
	// Raw curve: window_fullscreen_curve cubic-bezier(0.215, 0.61, 0.355, 1)
	if (strncmp(a, "cubic-bezier", strlen("cubic-bezier")) == 0) {
		...
		for (int i = 0; i < argc; ++i) {
			...
			if (used + len + 2 > sizeof(joined)) {
				return cmd_results_new(CMD_FAILURE,
					"window_fullscreen_curve cubic-bezier value too long");
			}
			if (used) {
				joined[used++] = ' ';
			}
		...
		return cmd_results_new(CMD_FAILURE,
			"window_fullscreen_curve cubic-bezier expects four floats with x in [0; 1]");
	}

	return cmd_results_new(CMD_INVALID, "window_fullscreen_curve: unknown curve");
}
